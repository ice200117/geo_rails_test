class MonitorPointYear < ActiveRecord::Base
	...
	def yesterday_by_cityid(cityid)
		tmp=MonitorPointYear.last.data_real_time
		time_start=tmp.beginning_of_day
		time_end=tmp.end_of_day
		City.find(cityid).monitor_point_years.where("data_real_time >= ? AND data_real_time <=?",time_start,time_end)
	end
	#重写保存(参数类型为哈希)
	def save_with_arg(d)
		return 'id or time is nil' if d['id'] == nil || d['time'] == nil
		if MonitorPointYear.where("monitor_point_id =? AND data_real_time >= ? AND data_real_time <= ?",d['id'], d['time'].beginning_of_day,d['time'].end_of_day).length == 0
			linedata=MonitorPointYear.new
