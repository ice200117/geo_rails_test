class TempLfHour < ActiveRecord::Base
    ...
    def atmospheric_volume_superscalar
        #大气容量超标计算
        #输出
        level2 = {'SO2'=>60,'NO2'=>40,'CO'=>4,'pm25'=>35,'pm10'=>70,'03'=>160}
        region = {
        ...
        data.group_by{|x| x.city_id}.each do |k,v|
            ...
            filter = {'SO2'=>0,'NO2'=>0,'CO'=>0,'O3'=>0,'pm25'=>0,'pm10'=>0,'AQI'=>0,'B'=>0}
            tmp = Array.new
            tmp << region[name][2]
            tmp << name
            filter.each do |m,n|
                v.each{|x| n+=x[m]}
                n /= v.size
                filter['B']=mix_height(filter['AQI'])
                next if m == 'AQI' or m == 'B'
                tmp << (n-level2[m])*region[name][0]*region[name][1]*10**-12
            end
            data << tmp
        end
        data
