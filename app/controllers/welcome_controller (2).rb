class WelcomeController < ApplicationController
  ...
  def chartway
    ...
    if  params[:starttime]==params[:endtime] 
      ...
      querydata=TempLfHour.where("data_real_time>=? AND data_real_time<=?",startdate,enddate)
    elsif params[:exact]=='eh'
      querydata=TempLfHour.where("data_real_time>=? AND data_real_time<=?",startdate,enddate) 
    else
      querydata=TempLfDay.where("data_real_time>=? AND data_real_time<=?",startdate,enddate)
    end  
...
=end
  ...
  @sfcitiesrankbyday=get_rank_json('rankdata','CHINARANK','DAY','')
